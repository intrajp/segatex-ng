...
static int read_response(int fd, char *buffer, size_t bufsz)
{
  ...
  while (r < bufsz)
  {
    printf("read_response while loop!\n");
    rc = read(fd, buffer + r, bufsz - r);
    if (rc == 0)
      {
        printf("read_response while loop rc = %d !\n",rc);
        break;
      }
    else if (rc > 0)
      {
    ...
    else
    {
      ...
      printf("read_response(): read() failed: %s\n", strerror(errno));
      return -1;
    }
  }
  return r;
...
static int wait_for_response(int fd)
{
  ...
  printf("DEBUG: wait_for_response(): i=%d, rc=%d\n", i, rc);
  if (i != sizeof(int))
  {
    //log_log(LOG_ERR, "wait_for_response(): read_response() returned %d (expected %d)",
    printf("wait_for_response(): read_response() returned %d (expected %d)\n",
            i, (int)sizeof(int));
    if (errno == 0)
#ifdef ENODATA
      errno = ENODATA;
  ...
  printf("DEBUG: wait_for_response(): i=%d, l=%d\n", i, l);
  if ((i != sizeof(int)) || (l <= 0))
    _exit(rc);
  /* read string */
